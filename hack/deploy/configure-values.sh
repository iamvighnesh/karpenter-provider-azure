...
CLUSTER_DISK_ENCRYPTION_SET_ID=$(jq -r ".diskEncryptionSetID // null" <<< "$AKS_JSON")
...
export CLUSTER_NAME AZURE_LOCATION AZURE_RESOURCE_GROUP_MC KARPENTER_SERVICE_ACCOUNT_NAME \
    CLUSTER_ENDPOINT BOOTSTRAP_TOKEN SSH_PUBLIC_KEY VNET_SUBNET_ID CLUSTER_DISK_ENCRYPTION_SET_ID KARPENTER_USER_ASSIGNED_CLIENT_ID \
    NODE_IDENTITIES AZURE_SUBSCRIPTION_ID NETWORK_PLUGIN NETWORK_PLUGIN_MODE NETWORK_POLICY LOG_LEVEL VNET_GUID
